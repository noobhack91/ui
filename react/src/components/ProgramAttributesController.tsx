...
import { Bahmni } from 'bahmni-util'; // Assuming Bahmni utilities are available in a similar manner
            ...
            <ul>
                {programAttributeTypes.map((attributeType) => (
                    isIncluded(attributeType) && (
                        <li key={attributeType.uuid}>
                            <strong>{attributeType.name}:</strong> {getValueForAttributeType(attributeType)}
                        </li>
                    )
                ))}
            </ul>
