...
const TopDownAddressFieldsDirectiveController: React.FC<TopDownAddressFieldsDirectiveControllerProps> = ({
    ...
    const isParentValueInvalid = (parentId: string) => {
        // SECOND AGENT: [MISSING CONTEXT] - Logic to check if parent value is invalid needs to be implemented here
    };
                ...
                <div key={index} className="address-level">
                    ...
                    <input
                        ...
                        onChange={(e) => {
                            ...
                            if (value) {
                                addressFieldSelected(addressLevel.addressField)({ addressField: { name: value, uuid: '', parent: null } });
                            } else {
                                clearFields(addressLevel.addressField);
                            }
                        }}
                        readOnly={isReadOnly(addressLevel)}
                        className={addressFieldInvalid ? 'illegalValue' : ''}
                    />
                </div>
            ))}
        </div>
