        ...
        <div>
            ...
            {addressLevels.map((level, index) => (
                <div key={index} className="address-level">
                    <label htmlFor={level.addressField}>{t(level.addressField)}</label>
                    <input
                        type="text"
                        id={level.addressField}
                        value={selectedValue[level.addressField] || ''}
                        onChange={(e) => {
                            const newValue = e.target.value;
                            setSelectedValue(prev => ({ ...prev, [level.addressField]: newValue }));
                            clearFields(level.addressField);
                        }}
                        readOnly={isReadOnly(level)}
                        onBlur={() => {
                            if (fieldValidation) {
                                fieldValidation(level.addressField, selectedValue[level.addressField]);
                            }
                        }}
                    />
                    <button
                        type="button"
                        onClick={removeAutoCompleteEntry(level.addressField)}
                    >
                        {t('Remove')}
                    </button>
                </div>
