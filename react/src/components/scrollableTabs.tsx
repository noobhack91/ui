...
const ScrollableTabs: React.FC = ({ children }) => {
    ...
    const scrollLeft = () => {
        if (scrollContainerRef.current && containerRef.current) {
            const scrollPos = Math.min(scrollContainerRef.current.scrollLeft + containerRef.current.offsetWidth / 3, 0);
    ...
    const scrollRight = () => {
        if (scrollContainerRef.current && containerRef.current) {
            const scrollingWidth = scrollContainerRef.current.scrollWidth - containerRef.current.offsetWidth;
            const scrollPos = Math.min(Math.abs(scrollContainerRef.current.scrollLeft - containerRef.current.offsetWidth / 3), scrollingWidth);
            scrollContainerRef.current.scrollTo({ left: scrollPos, behavior: 'smooth' });
            setShowLeft(true);
    ...
    return (
        <div className="scrollable-tabs" ref={containerRef}>
