...
const Timeline: React.FC<TimelineProps> = ({ program }) => {
    ...
    useEffect(() => {
        ...
        states.select(".label-bg")
            ...
            .attr('width', d => xMax - timeScale(d.date));
        states.select(".label")
            .attr('x', d => timeScale(d.date) + stateBar.textPaddingX)
            .attr('y', stateBar.y + (stateBar.height * 0.7))
            .text(d => d.state);
        states.select(".date-bg")
            .attr('x', d => timeScale(d.date) + dateBar.xPadding)
            .attr('y', dateBar.y)
