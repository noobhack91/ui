...
const search = (query: string, identifier: string, addressFieldName: string, addressFieldValue: string, customAttributeValue: string,
                offset: number, customAttributeFields: string, programAttributeFieldName: string, programAttributeFieldValue: string, addressSearchResultsConfig: any,
                patientSearchResultsConfig: any, filterOnAllIdentifiers: boolean) => {
    const config = {
        params: {
            q: query,
...
const get = (uuid: string) => {
    ...
    const data = {
        ...openMRSPatient,
        ...patient,
        attributes: $rootScope.patientConfiguration.attributeTypes.map((attrType: any) => {
            return {
                attributeType: attrType.uuid,
                value: patient[attrType.name]
            };
        })
    };
    const config = {
...
const create = (patient: any, jumpAccepted: boolean) => {
    ...
    const data = {
        ...openMRSPatient,
        ...patient,
        attributes: patient.attributeTypes.map((attrType: any) => {
            return {
                attributeType: attrType.uuid,
                value: patient[attrType.name]
            };
