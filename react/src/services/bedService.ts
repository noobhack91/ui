...
class BedService {
    ...
    public async setBedDetailsForPatientOnRootScope(uuid: string, setRootScope: (bedDetails: BedDetails | undefined) => void): Promise<BedDetails | undefined> {
        const bedDetails = await this.getAssignedBedForPatient(uuid);
        setRootScope(bedDetails);
        return bedDetails;
    }
    ...
    public async getBedInfo(bedId: string): Promise<any> {
        return await axios.get(`${BahmniCommonConstants.bedFromVisit}/${bedId}?v=custom:(bedId,bedNumber,patients:(uuid,person:(age,personName:(givenName,familyName),gender),identifiers:(uuid,identifier),),physicalLocation:(name))`, {
            withCredentials: true
        });
    }

    public async getCompleteBedDetailsByBedId(bedId: string): Promise<any> {
        return await axios.get(`${BahmniCommonConstants.bedFromVisit}/${bedId}`, {
            withCredentials: true
        });
    }
}
