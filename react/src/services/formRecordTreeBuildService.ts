...
class FormRecordTreeBuildService {
    ...
    createMultiSelectObservation(observations: Observation[]): Observation {
    ...
    preProcessMultiSelectObs(value: Observation[]): Observation[] {
    ...
    createObsGroupForForm(observations: Observation[], formBuilderForms: Form[]): Observation[] {
        observations.forEach(obs => {
            const newValues: Observation[] = [];
            obs.value.forEach(value => {
                if (!value.formFieldPath) {
                    newValues.push(value);
                    return;
                }
                const obsGroup: Observation = {
                    groupMembers: [],
                    concept: {
                        shortName: "",
                        conceptClass: null
                ...
                if (formNameTranslations.length > 0) {
                    const currentLabel = formNameTranslations
                        .find(formNameTranslation => formNameTranslation.locale === locale);
                    if (currentLabel) {
    ...
    getFormByFormName(formList: Form[], formName: string, formVersion: string): Form | undefined {
        return formList.find(form => form.name === formName && form.version === formVersion);
    }

    getFormName(members: Observation[]): string | undefined {
        const member = members.find(member => member.formFieldPath !== null);
        return member ? member.formFieldPath.split('.')[0] : undefined;
    }

    getFormVersion(members: Observation[]): string | undefined {
        const member = members.find(member => member.formFieldPath !== null);
        return member ? member.formFieldPath.split('.')[1].split('/')[0] : undefined;
    }

    updateObservationsWithRecordTree(formDef: any, form: Observation, translationData: any): Observation {
    ...
    createColumnGroupsForTable(record: any, columns: any[], tableGroup: Observation, obsList: Observation[], translationData: any) {
    ...
    getTranslatedShortName(translationData: any, translationKey: string, obsGroup: Observation, defaultShortName: string): string {
    ...
    isTranslationKeyPresent(translationData: any, translationKey: string): boolean {
        return translationData && translationData.labels &&
            translationData.labels[translationKey][0] !== translationKey;
    }

    getColumnObs(columnIndex: number, record: any): any[] {
    ...
    createGroupMembers(recordTree: any, obsGroup: Observation, obsList: Observation[], translationData: any) {
    ...
    getTableColumns(record: any): any[] {
        return record.control.columnHeaders.filter((child: any) => child.type === "label");
    }

    getRecordObservations(obsFormFieldPath: string, obsList: Observation[]): Observation[] {
        return obsList.filter(obs => obs.formFieldPath && obs.formFieldPath === obsFormFieldPath);
    }

    createObsGroup(record: any, translationData: any): Observation {
        const obsGroup: Observation = {
            groupMembers: [],
            concept: {
                shortName: "",
    ...
    getRecordTree(formDef: any, groupMembers: Observation[]): any {
